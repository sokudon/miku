...
--//! license : MIT
--//! github.com/moment/moment-timezone Pacific/Apiaだけ移植（）
PSTname="Pacific/Apia"
PSTabbrs={"LMT","LMT","-1130","-11","-10","-11","-10","+14","+13","+14","+13","+14","+13","+14","+13","+14","+13","+14","+13","+14","+13","+14","+13","+14","+13","+14","+13"}
PSTuntils={-2445424384000,-1861878784000,-631110600000,1285498800000,1301752800000,1316872800000,1325239200000,1333202400000,1348927200000,1365256800000,1380376800000,1396706400000,1411826400000,1428156000000,1443276000000,1459605600000,1474725600000,1491055200000,1506175200000,1522504800000,1538229600000,1554559200000,1569679200000,1586008800000,1601128800000,1617458400000,nil}
PSToffsets={-753.0666666666667,686.9333333333333,690,660,600,660,600,-840,-780,-840,-780,-840,-780,-840,-780,-840,-780,-840,-780,-840,-780,-840,-780,-840,-780,-840,-780}
...
function set_time_text()
	...
	local time_textps="!".. string.gsub(time_textq, "%%z", get_tzoffset(pstst*3600))
	...
	local time_textpe="!".. string.gsub(time_textq, "%%z", get_tzoffset(psten*3600))
...
function script_description()
	return "Sets a text source to act as a timer with advanced options. Hotkeys can be set for starting/stopping and to the reset timer.%TZ %SS %EE moment-timezone-with-data-10-year-range.js Pacific/Apia のみ移植されてます(実験的)"
--を備えたタイマーとして機能するようにテキスト ソースを設定します。ホットキーは開始/停止およびリセット タイマーに設定できます。%TZ %SS %EE moment-timezone-with-data-10-year-range.js Pacific/Apia のみ移植されてます(実験的)"
end
...
function script_defaults(settings)
	...
	obs.obs_data_set_default_string(settings, "start_text", "2024-02-10 15:00PST")
	obs.obs_data_set_default_string(settings, "stop_text", "2024-02-17 21:00PST")
	...
	obs.obs_data_set_default_string(settings, "title_text", "ほどかれた糸のその先に")
	obs.obs_data_set_default_string(settings, "time_text", "%Y-%m-%dT%H:%M:%S%z (%a)")
	obs.obs_data_set_default_string(settings, "para_text", "PST現在時間%UTC\nPST終了時間%EU\n経過時間%K\n残り時間%L\nイベント時間%I\n%T%P％\n%Q")
